...
void process_bubblegun_output(const string &file_path, GFAData &gfa_data)
{
    ...
    // Mappa inversa (Node -> Walks) per sapere quali walk controllare
    // Chiave (Key): ID del nodo
    // Valore (Value): lista degli ID delle Walk che contengono quel nodo
    // std::unordered_map<std::string, std::vector<std::string>> node_to_walks;
    // for (const auto& [w_id, walk] : gfa_data.walks) {
    //     for (const auto& seg : walk.segments) {
    //         node_to_walks[seg.first].push_back(w_id);
    //     }
    // }
    ...
    for (auto it = data.begin(); it != data.end(); ++it)
    {
        ...
        for (const auto &bubble : chain["bubbles"])
        {
            ...
            // Recuperiamo solo le walk che contengono lo start_node della bolla
            // auto it_map = node_to_walks.find(start_node);
            // if (it_map == node_to_walks.end()) continue;

            // // print it_map->second for debugging
            // for (const auto &w_id : it_map->second)
            // {
            //     std::cout << "Walk containing start_node " << start_node << ": " << w_id << "\n";
            // }

            // const std::vector<std::string>& candidates = it_map->second;

            // Iteriamo solo sulle walk interessate
            // for (const std::string& w_id : candidates)
